...
$(function () {
  ...
  var          hl,
         newsList = $('.team-headlines'),
    newsListItems = $('.team-headlines li'),
    firstNewsItem = $('.team-headlines li:nth-child(1)'),
      newsPreview = $('.team-preview'),
          elCount = $('.team-headlines').children(':not(.highlight)').index(),
         vPadding = (parseInt(firstNewsItem.css('padding-top').replace('px', ''), 10)) +
                    (parseInt(firstNewsItem.css('padding-bottom').replace('px', ''), 10)),
          vMargin = (parseInt(firstNewsItem.css('margin-top').replace('px', ''), 10)) +
                    (parseInt(firstNewsItem.css('margin-bottom').replace('px', ''), 10)),
         cPadding = (parseInt($('.team-content').css('padding-top').replace('px', ''), 10)) +
                    (parseInt($('.team-content').css('padding-bottom').replace('px', ''), 10)),
  ...
  function doEqualHeight(c) {
    ...
    if ($('.team-content:nth-child(' + c + ')').height() > newsPreview.height()) {
      newsPreview.height($('.team-content:nth-child(' + c + ')').height() + cPadding);
    }
  }
  ...
  $('.team-content').on('mouseover', function () {
    clearInterval(myTimer);
  });

  $('.team-content').on('mouseout', function () {
      doTimedSwitch();
  });
  ...
  function doClickItem() {
    ...
    newsListItems.on('click', function () {
      ...
      $('.team-content:nth-child(' + indexEl + ')').siblings().removeClass('top-content');
      $('.team-content:nth-child(' + indexEl + ')').addClass('top-content');
